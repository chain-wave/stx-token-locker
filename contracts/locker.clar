...
(define-map users-token-locks
    ...
    { token-addr: principal, lock-id: uint }
)

;; maps the number of tokens user has locked in locker
(define-map user-locked-tokens-count { user-addr: principal }  uint )
...
(define-map token-locks
    { token-addr: principal, lock-id: uint }
    {
        ...
        lock-owner: principal
    }
)

;; maps the pair to the number of token locks
(define-map locked-tokens-count { token-addr: principal }  uint )
...
;; lockToken
(define-public (lock-token (token-trait <ft-trait>) (amount uint) (unlock-block uint) (fee-in-stx bool) (secondary-token-trait <ft-trait>) (withdrawer principal)) 
  (begin     
    ...
    (let 
      ...
      (map-set token-locks { token-addr: token, lock-id: next-token-id } { lock-block: block-height, amount: amount, initial-amount: amount, unlock-block: unlock-block, lock-owner: sender })
      (map-set locked-tokens-count { token-addr: token } (+ next-token-id u1))

      ;; create user token lock record
